...
//
//  The iOS half of `api/OtpPolicy.kt`. Two numbers the server owns. The client holds them so it
//  can stop OFFERING an action the server would refuse — a disabled resend link reads better than
//  a 429 — but the server remains the authority, and where the two disagree the response wins.
//
//  WHY THIS FILE EXISTS AT ALL
//
//  Each of these was written down twice: once in `DevAuth`, back when the sign-up flow faked its
//  own verification, and once in the profile flow when email verification became real. Two
//  spellings of the same server rule is the failure this repository has already paid for elsewhere
//  — see the three primary buttons in the iOS CLAUDE.md. Raising `OTP_MAX_ATTEMPTS` to 6 should be
//  one edit here, not a search.
